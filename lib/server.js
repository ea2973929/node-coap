...
function CoAPServer(options, listener) {
  ...
  if (!this._options.forceCopyBlockBuffs || !isBoolean(this._options.forceCopyBlockBuffs)) {
    this._options.forceCopyBlockBuffs = parameters.forceCopyBlockBuffs
  }
  ...
  // Allow the user to specify their own cache management for block2 data
  if (options.cache &&
        typeof options.cache.store === "function" &&
        typeof options.cache.retrieve === "function" &&
        typeof options.cache.remove === "function" && 
        typeof options.cache.clear === "function") {
          this._cache = options.cache
  } else {
    if(options.cache) {
      // Potentially a critical server error, don't want this to silently fail
      throw new Error("Options contains cache entry however is missing at least one critical function")
    }
    this._cache = new CacheManager(parameters.exchangeLifetime * 1000)
  }
...
CoAPServer.prototype._handle = function(packet, rsinfo) {
  ...
  var generateResponse = function() {
    var responseMsg = new Message(packet, 
      /**
       * Send function. Called from outgoing_message, passed 
       * @param {OutgoingMessage} response
       */
      function(response, packet) {
        var buf;
        var sender = new RetrySend(sock, rsinfo.port, rsinfo.address)
        ...
        var key = toKey(rsinfo.address, rsinfo.port, packet, packet.ack || !packet.confirmable)
        lru.set(key, buf)
        buf.sender = sender
    ...
    responseMsg.statusCode = '2.05'
    responseMsg._request = request._packet
    responseMsg._cachekey = toCacheKey(rsinfo.address, rsinfo.port, packet)
    responseMsg._forceCopyBlockBuffs = that._options.forceCopyBlockBuffs;
    //inject this function so the response can add an entry to the cache
    responseMsg._addCacheEntry = function(key, val) {
      that._cache.store(key, val, request);
    }
  ...
  if (packet.token && packet.token.length > 0) {
    ...
    var cachedDataB2 = this._cache.retrieve(response._cachekey)
    var cachedDataExistsB2 = cachedDataB2 !== null && cachedDataB2 !== undefined;

    if (requestedBlockOption.num < 1) {
      if (cachedDataExistsB2) {
        this._cache.remove(response._cachekey)
      }
    } else {
      debug('check if packet token is in cache, key:', response._cachekey)
      if (cachedDataExistsB2) {
        debug('found cached payload, key:', response._cachekey)
        response.end(cachedDataB2)
...
OutMessage.prototype.end= function(payload) {
  ...
  var blockBuff;
  var start = (requestedBlockOption.num)*requestedBlockOption.size;
  var end = (requestedBlockOption.num+1)*requestedBlockOption.size;

  // cache it
  if (this._request.token && this._request.token.length > 0) {
    this._addCacheEntry(this._cachekey, payload)
    // Because we're caching the entire payload, slicing it is better
    // as we're not re-allocating memory
    blockBuff = payload.slice(start, end);
  } else {
    // Provide 'forceCopyBlockBuffs: true' to coap.createServer options
    // to enable this flag
    if(this._forceCopyBlockBuffs) {
      // If the payload isn't cached, then slicing the buffer
      // still retains the original buffer. Which can be bad, e.g.
      //   If the server is reading a 1MB file from filesystem every request,
      //   this 1 MB file requires about 1000 block transfers. Which means
      //   the file will be read 1000 times, and stored in memory, 1000 times.
      //   Thus, 1GB memory allocation, for one MB file over block2!
      // Ideally, this 1MB file is cached before the coap library handles it,
      // however sometimes this isn't possible.
      if(end > payload.length) {
        end = payload.length;
      }
      blockBuff = Buffer.alloc(end-start);
      payload.copy(blockBuff, 0, start, end);
      payload = null;
    } else {
      blockBuff = payload.slice(start, end);
    }
  ...
  OutgoingMessage.prototype.end.call(this, blockBuff)
};

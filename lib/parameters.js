...
var p = {
  ...
  // true:  If the block2 transfer isn't cached (because client is missing a token)
  //         then instead of slicing the payload, just copy what we need
  // false: Always slice the given buffer. This is good when you can guarantee that
  //         the given buffer {res.end(buff)} will remain in memory anyway, regardless
  //         of what this coap library does
  , forceCopyBlockBuffs: false
}
